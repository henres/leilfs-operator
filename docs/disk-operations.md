...
This guide covers common disk maintenance procedures for a LeilFS cluster managed by the operator.
...
LeilFS continuously monitors chunk availability. When a disk fails, the master detects missing chunks
...
1. Wait for LeilFS to finish re-replicating the affected chunks (monitor via `saunafs-admin`
   or the web UI).
...
5. LeilFS will gradually fill the new disk as new writes arrive and as the rebalancer redistributes
   existing chunks.

> **No CRD change is needed** as long as the replacement disk is mounted at the same `hostPath`.
> If the path changes, update the corresponding `mountPaths` entry in the `LeilFSCluster` resource
...
`hostPath`, LeilFS will automatically prefer it for new writes (the master allocates chunks
...
3. The chunk server re-registers with the master. LeilFS starts routing new writes to the
...
2. Add the new entry to `spec.chunk.servers` in the `LeilFSCluster` resource.
3. The operator creates a new StatefulSet for this chunkserver only — existing pods are not restarted.
4. LeilFS registers the new chunkserver and starts routing writes to it.
